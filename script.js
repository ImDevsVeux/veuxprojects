document.addEventListener('DOMContentLoaded', function() {
    ...
    function setupLoggedInUI() {
        if (startBtn) {
            ...
            console.log("Logged-in UI: Start button set to 'Let's Start!' and visible");
        }
        if (discordBtn) {
            discordBtn.style.display = 'none';
        }
        if (authButtons) {
            authButtons.style.display = 'flex'; // Ensure the container remains visible
        }
    ...
    function setupLoggedOutUI() {
        if (startBtn) {
            ...
            startBtn.style.display = 'flex';
            console.log("Logged-out UI: Start button set to 'Start!!' and visible");
        }
        if (discordBtn) {
    ...
    // Debug button visibility after 2 seconds
    setTimeout(() => {
        if (startBtn) {
            console.log("Start button visibility after 2s:", startBtn.style.display, startBtn.innerHTML);
        }
        if (authButtons) {
            console.log("Auth buttons container visibility after 2s:", authButtons.style.display);
        }
    }, 2000);
    ...
    function setupAnimationEditorInterface() {
        ...
        // Add "Add More Animations" button
        const addAnimBtn = document.createElement('button');
        addAnimBtn.textContent = 'Add More Animations';
        addAnimBtn.className = 'btn add-btn';
        addAnimBtn.id = 'add-anim-btn';
        animationContainer.appendChild(addAnimBtn);
        
        let animGroupCount = 5; // Start after the initial 5
        ...
        addAnimBtn.addEventListener('click', function() {
            if (animGroupCount < 15) {
                ...
                animationGroup.className = 'animation-group';
                animationGroup.innerHTML = `
                    <h3>Animation ${animGroupCount}</h3>
                    <div class="input-group">
                        <label>Original Animation ID</label>
                        <input type="text" class="original-anim" placeholder="Enter original animation ID">
                    </div>
        ...
        skipAnimBtn.addEventListener('click', function() {
            // Store animation data before proceeding
            ...
            if (skipAnimBtn.textContent === 'Done') {
                ...
                // No partial inputs - generate script
                if (sessionStorage.getItem('discord_token')) {
                    animationEditorInterface.classList.add('hidden');
                    vfxInterface.classList.remove('hidden');
                    setupVFXInterface();
...
humanoid.AnimationPlayed:Connect(onAnimationPlayed)`;
            ...
            // Add VFX script for signed-in users
            if (sessionStorage.getItem('discord_token')) {
                ...
                vfxGroups.forEach((group, index) => {
                    ...
                    if (path && moveIndex) {
                        // Find replacement animation ID for the selected move
                        let replacementId = "";
                        if (animationData[moveIndex - 1]) {
                            replacementId = animationData[moveIndex - 1].replacement;
                        }
