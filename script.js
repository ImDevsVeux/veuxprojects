document.addEventListener('DOMContentLoaded', function() {
    ...
    // AI Configuration - Using environment variable from GitHub Secrets
    const AI_API_URL = "https://router.huggingface.co/v1/chat/completions";
    const AI_MODEL = "meta-llama/Llama-3.1-8B-Instruct:fireworks-ai";
    const HF_TOKEN = process.env.HF_TOKEN || ""; // Will be injected via GitHub Secrets
    let messageCount = 0;
    const MAX_MESSAGES_GUEST = 15;
    ...
    function initializeChat() {
        ...
        addChatMessage('assistant', "Hey ! I'm Ur Ai Assistant. What Help u need ?");
        messageCount = 0;
        updateChatCounter();
    }
    ...
    sendChatBtn.addEventListener('click', async function() {
        ...
        if (!sessionStorage.getItem('discord_token') && messageCount >= MAX_MESSAGES_GUEST) {
            addChatMessage('assistant', "You've reached the maximum number of messages for guests. Please sign in to continue chatting.");
            return;
        }
    ...
    function updateChatCounter() {
        if (sessionStorage.getItem('discord_token')) {
            chatCounter.textContent = "Messages: Unlimited";
        } else {
            chatCounter.textContent = `Messages: ${messageCount}/${MAX_MESSAGES_GUEST}`;
        }
    }
    ...
    function generateScript() {
        ...
        // Remove signup message for logged-in users
        if (sessionStorage.getItem('discord_token')) {
